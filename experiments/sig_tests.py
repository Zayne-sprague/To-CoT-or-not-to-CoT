...
def bernoulli(data_A, data_B, alpha):
    confidence_level = 1.0 - alpha
    zval = stats.norm.ppf((1 + confidence_level) / 2)

    Aprime = np.mean(data_A)
    Avar = (Aprime * (1 - Aprime)) / len(data_A)

    Bprime = np.mean(data_B)
    Bvar = (Bprime * (1 - Bprime)) / len(data_B)

    SE = np.sqrt(Avar + Bvar)
    t = zval * SE
    delta = abs(Aprime - Bprime)

    calculated_z_score = delta / SE
    p_value = 2 * (1 - stats.norm.cdf(calculated_z_score))

    return p_value


def sigtest(data_A, data_B, alpha, name='t-test'):
    name = name.lower()
    # if len(sys.argv) < 3:
    ...
    if (name=="bernoulli"):
        pval = bernoulli(data_A, data_B, alpha)
        if (float(pval) <= float(alpha)):
            # print("\nTest result is significant with p-value: {}".format(pval))
            return True, pval
        else:
