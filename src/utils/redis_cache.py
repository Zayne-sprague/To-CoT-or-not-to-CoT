...
class RedisCacheCTXMgr:
    def __init__(self, redis_cache):
        self.redis_cache = redis_cache
        self.previous_keystore = None

    def __enter__(self):
        self.previous_keystore = deepcopy(self.redis_cache.keystore)

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.redis_cache.keystore = deepcopy(self.previous_keystore)
...
class RedisCache:
    ...
    static_context: "RedisCacheCTXMgr"
    ...
    def __init__(
        ...
        self.static_context = RedisCacheCTXMgr(self)
    ...
    def enable(
        ...
        try:
            if username:
                self.redis_backend = redis.StrictRedis(host=host, username=username, port=port, db=db)
            else:
            ...
            if not self.bust_cache and not self.disabled and not no_cache:
                if self.redis_backend.exists(key):
                    ...
                    try:
                        ...
                        # if isinstance(out, list) and not out[0].get("additional_info"):
                        #     pass
                        if out is not None and ( isinstance(out, dict) and out.get("API Error")) or (isinstance(out, list) and out[0] is not None and isinstance(out[0], dict) and out[0].get('API Error')):
                            pass
                        else:
                            return out
