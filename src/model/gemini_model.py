...
# TODO - Idk why it just started logging every single request so
import logging
...
class GeminiModel(Model):
    ...
    def __init__(
            self,
            engine: str = 'google/gemini-1.5-flash-001',
        ...
        vertexai.init(project=KeyHandler.google_project_id, location=KeyHandler.google_project_location)
        ...
        self.client = openai.OpenAI(
            base_url=f"https://{KeyHandler.google_project_location}-aiplatform.googleapis.com/v1beta1/projects/{KeyHandler.google_project_id}/locations/{KeyHandler.google_project_location}/endpoints/openapi",
    ...
    def __update_cost__(self, raw):
        if self.prompt_cost and self.completion_cost:
            ...
            self.total_prompt_tokens += raw.usage.prompt_tokens
            self.total_gen_tokens += raw.usage.completion_tokens
    ...
    def __safe_openai_chat_call__(
        ...
        if kwargs.get("vllm_sampling_kwargs"):
            del kwargs["vllm_sampling_kwargs"]
...
if __name__ == "__main__":
    ...
    raw = model.inference("Hello, how are you?")
    out = model.parse_out(raw)[0]
    print(out)