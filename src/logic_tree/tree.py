...
class LogicTree:
    ...
    def get_facts(self, include_cs: bool = False, include_deductions_from_level: int = -1, no_facts_after_depth: int = -1, include_depth: bool = False):
        """
    ...
    def print_tree(self, node=None, level=0):
        ...
        if node is None:
            node = self.nodes[0]
        line = '.' * level * 1 + str(node) + (" | " + str(node.operator) if len(node.children) > 0 else '')
    ...
    def print_for_gpt(
            ...
            ignore_value_before_depth: int = -1,
            print_only_nodes_with_value: bool = False,
            only_print_values: bool = False,
    ):
        """
        ...
        ignore_val = ignore_val or (ignore_value_before_depth > -1 and ignore_value_before_depth > level)
        ignore_line = print_only_nodes_with_value and (node.value == '' or ignore_val)

        if not print_forward:
            for child in node.children:
                v = self.print_for_gpt(child, level + 1, space_level=space_level+1 if not ignore_val else space_level, pad_char=pad_char, pad_space=pad_space, print_forward=print_forward, ignore_value_after_depth=ignore_value_after_depth, ignore_value_before_depth=ignore_value_before_depth, print_only_nodes_with_value=print_only_nodes_with_value, only_print_values=only_print_values)
                if v != '':
                    line += v + '\n'
        ...
        if ignore_line:
            ...
            line += pad_char * space_level * pad_space + line_val
